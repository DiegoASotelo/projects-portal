...
const renderCards = () => {
  ...
  buildSections(filteredCards()).forEach(section => {
    ...
    section.items.forEach(card => {
      ...
      node.querySelector('.thumb-button').onclick = () => { state.lastScrollY = window.scrollY; el.modalImage.src = cardImage(card); el.modalImage.classList.remove('rotated-landscape'); el.imageModal.showModal(); };
